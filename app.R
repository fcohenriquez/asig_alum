...
f_asig <- function(x,y,z,w){
  ...
  return(result1)
   ...
   output$asig_curso <- renderDataTable({ 
     ...
     f_asig(input$alumnos,input$n_curs_orig, input$n_curs_fin, input$n_comp)
  ...
  output$downloadData <- downloadHandler(
     ...
     content = function(file) {
       write.csv(f_asig(input$alumnos,input$n_curs_orig, input$n_curs_fin, input$n_comp), file, row.names = TRUE)
     }
   )
