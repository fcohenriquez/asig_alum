...
for (i in c(1:num_cursos)) {
  j=i+2
  vno <- paste("V",j, sep="")
  vnn<- paste("Curso ",i, sep="")
  colnames(result)[colnames(result)==vno] <- vnn

}

for (i in c((num_cursos+1):(num_cursos+num_escogidos))) {
  j=i+2
  vno <- paste("V",j, sep="")
  vnn<- paste("Escogido ",(i-num_cursos), sep="")
  colnames(result)[colnames(result)==vno] <- vnn
...
rm(bloque_curso, aux, aux1, aux2, i, j, k, vnn, vno, id_alum)

sapply(result, sum)
aggregate(. ~ ninas, data=result, sum)
aggregate(. ~ curso_orig, data=result, sum)


write.csv(result, file = "asignacion.csv")
