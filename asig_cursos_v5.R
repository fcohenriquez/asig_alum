...
datos_entrada <-read.csv("datos_entrada.csv",row.names=1)
resultados <- f_asig("datos_entrada.csv", 3)

a <- table(resultados$curso_orig,resultados$curso_final)
union <- merge(datos_entrada,resultados, by=c("id", "id"))

table (union$curso_final,union$ninas)
table (union$curso_orig.x,union$curso_final )
